...
func (rm *resourceMonitor) Scan(excludeList ResourceExcludeList) (topologyv1alpha1.ZoneList, error) {
	...
	for nodeID, resCounters := range rm.nodeCapacity {
		...
		for resName, resAlloc := range resCounters {
			...
			allocatableQty := *resource.NewQuantity(resAlloc-resUsed, resource.DecimalSI)
...
func (rm *resourceMonitor) updateNodeCapacity() error {
	...
	allDevs := NormalizeContainerDevices(allocRes.GetDevices(), allocRes.GetCpuIds(), rm.coreIDToNodeIDMap)
...
func GetAllContainerDevices(podRes []*podresourcesapi.PodResources, namespace string, coreIDToNodeIDMap map[int]int) []*podresourcesapi.ContainerDevices {
	...
	for _, pr := range podRes {
		...
		for _, cnt := range pr.GetContainers() {
			allCntRes = append(allCntRes, NormalizeContainerDevices(cnt.GetDevices(), cnt.GetCpuIds(), coreIDToNodeIDMap)...)
		}
...
func NormalizeContainerDevices(Devices []*podresourcesapi.ContainerDevices, CpuIds []int64, coreIDToNodeIDMap map[int]int) []*podresourcesapi.ContainerDevices {
